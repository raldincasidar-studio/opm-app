...
export default {
  ...
  async mounted() {
    this.$axios.get('/get_balance?id='+this.auth.id).then(res => {
      ...
      this.history = res.data.record_summary;
...
div.page
  Navbar(title="Balance" back="true")
  .image 
  .contents

    h5.stall PHP {{balance}}
    h6.name Current Balance
